...
const userDataInfoDom = document.querySelector(".user-data-info");
$.ajax({
	type: 'GET',
	...
	success:function(data){
		...
		for(let i=0;i<data.length;i++){
			userData = userData + `<li data-id='${data[i].userId}' data-name='${data[i].username}'>
								<div class="surname">
									<p>${(data[i].username).substring(0,1)}</p>
								</div>
								<div class="user-info">
									<p>用户名称：${data[i].username}</p>
									<p>用户编号：${data[i].userId}</p>
								</div>
							</li>`;
		}
		userDataInfoDom.innerHTML = userData;
...
function updatePart3(){
	const userUlDoms = document.querySelector(".user-content ul");
	const userLiDoms = document.querySelectorAll(".user-content ul li");
	
	if(userLiDoms.length>0){
		$.ajax({
			type: 'POST',
			url: 'http://101.132.125.83:8081/arm/userScoreAllGroup',
			data: {userId: userLiDoms[0].getAttribute("data-id")},
			dataType: 'json',
			success:function(data){
				let id = userLiDoms[0].getAttribute("data-id");
				if(typeof(id) != 'undefined' && id != null){
					updateTable(data, userLiDoms[0].getAttribute("data-id"), userLiDoms[0].getAttribute("data-name"));
				}
			},
			error:function (e) {
	...
	let clickIndex = 0;
	for(let i=0;i<userLiDoms.length;i++){
		userLiDoms[i].addEventListener("click", function(){
			userLiDoms[clickIndex].setAttribute("style", "background: #181827;");
			userLiDoms[i].setAttribute("style", "background: #222437;");
			clickIndex = i;
			
			let uid = userLiDoms[i].getAttribute("data-id");
			let uname = userLiDoms[i].getAttribute("data-name");
			$.ajax({
				type: 'POST',
				url: 'http://101.132.125.83:8081/arm/userScoreAllGroup',
				data: {userId: uid},
