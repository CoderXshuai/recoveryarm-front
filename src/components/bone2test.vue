  ...
  export default {
    ...
    methods: {
      ...
      initBones() {
		...
		xhr.open('get','http://101.132.125.83:8081/arm/dataQuater/14');
		xhr.send();
      ...
      createAnimation() {
		...
		// 	tArray[i] = this.getQuater(((data[i].tza-data[0].tza) / 180) * Math.PI, -((data[i].txa-data[0].txa) / 180) * Math.PI, ((data[i].tya-data[0].tya) / 180) * Math.PI);
		// 	bArray[i] = this.getQuater(0, 0, ((data[i].bya-data[0].bya) / 180) * Math.PI);
		// }
		let txyz = new Array();
		let bxyz = new Array();
		for(let i=0;i<count;i++){
			txyz.push(data[i].tq0-data[0].tq0, data[i].tq1-data[0].tq1, data[i].tq2-data[0].tq2, data[i].tq3-data[0].tq3+1);
			bxyz.push(data[i].bq0-data[0].bq0, data[i].bq1-data[0].bq1, data[i].bq2-data[0].bq2, data[i].bq3-data[0].ba3+1);
		}
      ...
      setupDatGui() {
		...
		// const ax = document.querySelector(".angle_x");
		// const ay = document.querySelector(".angle_y");
		// const az = document.querySelector(".angle_z");
		// const avx = document.querySelector(".angle_vx");
		// const avy = document.querySelector(".angle_vy");
		// const avz = document.querySelector(".angle_vz");
		// const aax = document.querySelector(".angle_ax");
		// const aay = document.querySelector(".angle_ay");
		// const aaz = document.querySelector(".angle_az");
		// const ti = document.querySelector(".exer_time");
		
		// let axa = [-13.826,-2.626,-7.169,-5.273,-7.092,-13.826,-2.626,-7.169,-5.273,-7.092];
		// let aya = [-71.735,-69.922,-67.385,-63.259,-60.062,-58.585,-60.474,-64.232,-70.631,-63.528];
		// let aza = [146.157,145.931,144.662,141.185,138.345,136.983,135.775,132.413,135.082,137.340];
		// let avxa = [3.784,-0.488,0.549,3.784,8.179,2.869,4.028,9.460,4.456,-2.869];
		// let avya = [0.549,0.366,-3.418,3.662,6.714,4.883,-0.183,-0.183,-6.348,2.625];
		// let avza = [-0.610,0.793,-0.610,1.038,-0.366,2.502,-2.014,0.610,1.221,1.404];
		// let aaxa = [0.942,0.940,0.883,0.850,0.838,0.877,0.939,0.945,0.967,0.859];
		// let aaya = [-0.070,-0.041,-0.062,-0.036,-0.011,-0.025,0.004,-0.011,-0.027,-0.040];
		// let aaza = [0.314,0.321,0.356,0.409,0.537,0.515,0.500,0.468,0.315,0.354];
		// let axi=0,ayi=0,azi=0,avxi=0,avyi=0,avzi=0,aaxi=0,aayi=0,aazi=0;
		// let mes = 0;
        ...
        gui.add(options, 'run').onChange(e => {
          if (e) {
			...
			// interval = setInterval(function(){
				// ax.innerText=axa[(axi++)%10];
				// ay.innerText=aya[(ayi++)%10];
				// az.innerText=aza[(azi++)%10];
				// avx.innerText=avxa[(avxi++)%10];
				// avy.innerText=avya[(avyi++)%10];
				// avz.innerText=avza[(avzi++)%10];
				// aax.innerText=aaxa[(aaxi++)%10];
				// aay.innerText=aaya[(aayi++)%10];
				// aaz.innerText=aaza[(aazi++)%10];
			// }, 100);
